...
## XCT Claw runtime — read this first

This skill is invoked from a XCT Claw agent. The host runtime has its
...
chat instead of being available the next turn).
...
it without a pod restart.
  ...
  ❌ write_file(path="/skills/domain-check/SKILL.md", ...) → sandbox overlay,
       lost on next rebuild
...
message" so they don't expect immediate use.
...
etc. On XCT Claw, ignore those file-layout details: the only thing
...
those scripts are read-only references, you don't need them to ship a
basic skill.

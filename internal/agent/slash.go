...
func (a *Agent) handleSlashCommand(msg bus.InboundMessage) slashResult {
	...
	case "/version":
		return slashResult{handled: true, reply: fmt.Sprintf("⚡ XCT Claw\nAgent: %s\nModel: %s", a.name, a.model)}
...
//
// Web / api: the chatter's UserID is the XCT Claw user UUID — owner is
...
// no inherent link to the agent's XCT Claw owner. The owner registers
...
func (a *Agent) isAdminChatter(msg bus.InboundMessage) bool {
	// Web / api carry XCT Claw UUIDs directly; owner check is sufficient.
...
func (a *Agent) slashStatus(msg bus.InboundMessage) slashResult {
	...
	status := fmt.Sprintf("⚡ XCT Claw Status\n"+
		"─────────────────\n"+
		"Agent:       %s\n"+
		"Model:       %s\n"+
		"Personality: %s\n"+
